google.charts.setOnLoadCallback(function() {
	var query = "SELECT tbl_companies.CompanyName AS `Company`, Count(tbl_signups_18.CompanyUnit) AS `Count` FROM `tbl_signups_18` INNER JOIN `tbl_companies` ON tbl_signups_18.CompanyUnit = tbl_companies.CompanyID WHERE tbl_signups_18.RegisteeType < 5 GROUP BY Company ORDER BY Company";
	convertCSVToPieChart(query, "Total Registrations", "recentStatistics_total", true);

	var chartQuery = "SELECT DateRegistered AS Date, Count(DateRegistered) AS `Number of Registrations` FROM `tbl_signups_18` GROUP BY Date ORDER BY Date";
